...
const RecommendationSection: React.FC<Props> = ({ recommendation }) => {
  ...
  const companyLogo = baseImageUrl + recommendation.companyLogo.data.attributes.url;

  return (
    <>
      <section className="flex flex-col items-center gap-10 w-full mx-auto py-20 bg-third">
        <div className="flex gap-2">
          ...
          <h1 className="text-4xl text-primary">{recommendation.companyTitle}</h1>
        </div>
          ...
          <div className="animate-scroll flex gap-24 w-full whitespace-nowrap">
            {recommendation.companyIcon.map((icon) => (
              <div key={icon.id} className="">
                <img src={baseImageUrl + icon.logo.data.attributes.url} alt={icon.logo.data.attributes.alternativeText || "Company Logo"} className="w-full object-contain" />
              </div>
            ))}
          </div>
        </div>
